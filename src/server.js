...
// tender-mcp's proven implementation (live on mainnet as of this port).
...
const TOOL_PRICES = { validate_data_safety: '$0.025' };
...
// Gate hits (free-tier exhausted, paid-limit exhausted) return before the
// normal success-path counters run — this makes them visible as EVENTS to
...
if (X402_ENABLED) {
  const validateDataSafetyTool = tools.find(t => t.name === 'validate_data_safety');
  X402_DISCOVERY_EXTENSIONS.validate_data_safety = declareDiscoveryExtension({
    toolName: 'validate_data_safety',
    description: validateDataSafetyTool.description.slice(0, 500),
    inputSchema: validateDataSafetyTool.inputSchema,
    example: { payload: 'Customer email: jane@example.com, SSN: 123-45-6789', context: 'send to third-party API' },
    output: { example: { verdict: 'REDACT_BEFORE_PASSING', sensitivity_level: 'CONFIDENTIAL', patterns_detected: ['SSN', 'EMAIL'], applicable_regulations: ['GDPR', 'CCPA'] } }
  });
}
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (email) {
        ...
        if (emailCircuitBreakerAllow()) {
          ...
          console.error('[EmailBreaker] suppressed API key delivery email for ' + email + ' — hourly cap reached, key is still valid, follow up manually');
        }
    ...
    req.on('end', async () => {
      try {
        ...
        if (!trialGrantCircuitBreakerAllow()) {
          res.writeHead(200, { ...cors, 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ granted: false, error: 'Trial extension grants are temporarily rate-limited on this server.', message: 'Please try again in a few minutes, or upgrade now: ' + STRIPE_PRO_URL, upgrade_url: STRIPE_PRO_URL, agent_action: 'RETRY_LATER_OR_UPGRADE' }));
          return;
        }
        ...
        if (request.method === 'initialize') {
          ...
          if (!access.allowed) {
            ...
            if (X402_ENABLED && access.tier === 'free_limit_reached' && TOOL_PRICES[name]) {
              try {
                ...
                const paymentRequired = await x402Server.createPaymentRequiredResponse(built, { url: 'https://data-compliance-mcp-production.up.railway.app', description: 'Data Compliance MCP — ' + name, mimeType: 'application/json' }, undefined, X402_DISCOVERY_EXTENSIONS[name]);
          ...
          if (access.tier === 'x402') {
            ...
            sendEmail(
              ...
              '[x402 SETTLEMENT] Data Compliance — real payment received',
